<template>
    <n-drawer v-model:show="isActive" :width="drawerWidth" class="!rounded-tl-xl !rounded-bl-xl overflow-y-auto">
      <div class="text-[1rem] font-medium tracking-wider mb-4 bg-[#FF6F6E] text-white px-4 sm:py-3 py-2 text-center flex items-center justify-center">收藏景點</div>
        ...
        <div class="w-[180px] h-[70px]">
          <img class="rounded-lg w-full h-full object-cover pointer-events-none" :src="item.Picture1" :alt="item.Name">
        </div>
          <div class="pl-2 w-full">
            ...
            <div class="text-[0.7rem] text-blue-500 cursor-pointer underline pt-1">詳細資訊</div>
          </div>
        </div>
    </n-drawer>
</template>
