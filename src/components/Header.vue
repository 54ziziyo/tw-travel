  ...
  <header>
    ...
    >
      ...
      <img
        ...
        :class="{ 'opacity-0': !backgroundImageLoaded }"
      />
      <router-link to="/">
        <div class="cursor-pointer absolute left-0 top-0">
          <img
            alt="Vue logo"
            class="sm:w-[12rem] w-[8rem] pointer-events-none"
      ...
      <div
        v-if="regionTitle"
        class="text-white font-serif flex items-center sm:space-x-8 space-x-4 relative z-10"
      >
      ...
      <div
        class="absolute sm:right-24 sm:top-8 right-4 top-4 cursor-pointer z-10"
        @click="activate('right')"
      >
        <div
          v-if="store.treasures.length > 0"
          class="absolute -right-2 top-0 bg-[#da0000] px-2 text-right text-white rounded-full"
        >
...
const backgroundImage = computed(() => {
  return props.imgUrl || '/homeNavBg.png'
})
